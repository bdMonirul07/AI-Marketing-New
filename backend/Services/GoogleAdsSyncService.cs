    ...
    public class GoogleAdsSyncService : BackgroundService
    {
        ...
        public async Task<GoogleSyncSummary> SyncGoogleAdsForCompany(int companyId, CancellationToken ct = default)
        {
            ...
            try
            {
                ...
                // Try metrics directly; if account is a manager, fall back to sub-account sync
                try
                {
                    summary.MetricRows = await SyncMetricsAsync(db, companyId, customerId, developerToken!, accessToken, loginCustomerId: null, ct);
                    await db.SaveChangesAsync(ct);
                }
                catch (InvalidOperationException ex) when (ex.Message.Contains("REQUESTED_METRICS_FOR_MANAGER"))
                {
                    _logger.LogInformation("[GOOGLE SYNC] Account {CustomerId} is a manager account — discovering client sub-accounts", customerId);
                    var clientIds = await GetClientAccountIdsAsync(customerId, developerToken!, accessToken, ct);

                    if (clientIds.Count == 0)
                    {
                        _logger.LogWarning("[GOOGLE SYNC] No client accounts found under manager {CustomerId} — cannot fetch metrics", customerId);
                    }
                    else
                    {
                        _logger.LogInformation("[GOOGLE SYNC] Found {Count} client accounts: [{Ids}]", clientIds.Count, string.Join(", ", clientIds));
                        foreach (var clientId in clientIds)
                        {
                            try
                            {
                                var rows = await SyncMetricsAsync(db, companyId, clientId, developerToken!, accessToken, loginCustomerId: customerId, ct);
                                summary.MetricRows += rows;
                                await db.SaveChangesAsync(ct);
                                _logger.LogInformation("[GOOGLE SYNC] Client {ClientId}: {Rows} metric rows", clientId, rows);
                            }
                            catch (Exception clientEx)
                            {
                                _logger.LogWarning(clientEx, "[GOOGLE SYNC] Failed metrics for client account {ClientId}", clientId);
                            }
                        }
                    }
                }
        ...
        // Enumerate direct client accounts (level=1) under a manager account.
        private async Task<List<string>> GetClientAccountIdsAsync(
            string managerId, string developerToken, string accessToken, CancellationToken ct)
        {
            const string query = @"
                SELECT customer_client.id, customer_client.manager, customer_client.level, customer_client.descriptive_name
            ...
            try
            {
                var rows = await ExecuteGaqlAsync(managerId, developerToken, accessToken, query, ct, loginCustomerId: null);
                var ids = rows
                    .Select(r => GetGaqlString(r, "customerClient", "id"))
                    .Where(id => !string.IsNullOrWhiteSpace(id) && id != managerId)
                    .Cast<string>()
                    .Distinct()
                    .ToList();
                return ids;
            ...
            {
                _logger.LogWarning(ex, "[GOOGLE SYNC] Could not enumerate client accounts for manager {ManagerId}", managerId);
        ...
        // ── Metrics sync ─────────────────────────────────────────────────────────
        // loginCustomerId: set to the manager account ID when querying a sub-account on behalf of a manager.
        private async Task<int> SyncMetricsAsync(AppDbContext db, int companyId, string customerId,
            string developerToken, string accessToken, string? loginCustomerId, CancellationToken ct)
        {
            // segments.hour gives one row per (ad, date, hour) instead of a daily cumulative total.
            const string query = @"
            ...
            var rows = await ExecuteGaqlAsync(customerId, developerToken, accessToken, query, ct, loginCustomerId);
            ...
            foreach (var row in rows)
            {
                ...
                var hourStr = GetGaqlString(row, "segments", "hour");

                if (!campaignMap.TryGetValue(googleCampaignId ?? "", out var dbCampaignId)) continue;
                if (!DateTime.TryParse(dateStr, CultureInfo.InvariantCulture, DateTimeStyles.AdjustToUniversal, out var date)) continue;

                // Build an hourly timestamp: date + hour so each row is a distinct hour bucket.
                var hour = int.TryParse(hourStr, out var h) ? h : 0;
                date = DateTime.SpecifyKind(date.Date.AddHours(hour), DateTimeKind.Utc);
        ...
        private async Task<GoogleSyncSummary> GenerateSimulatedGoogleSync(AppDbContext db, int companyId, CancellationToken ct)
        {
            ...
            {
                // Ensure a simulated ad set exists for this campaign
                var adSets = await db.AdSets
                    .Where(a => a.CampaignId == campaign.Id && a.CompanyId == companyId && a.Status != "archived")
                    .ToListAsync(ct);

                if (adSets.Count == 0)
                {
                    var simAdSet = new AdSet
                    {
                        CompanyId = companyId,
                        ...
                        OptimizationGoal = "CLICKS",
                        BillingEvent = "CLICKS",
                        GoogleAdGroupId = $"sim_{campaign.Id}_ag1",
                        PlatformAdSetIds = JsonSerializer.SerializeToElement(new { google_ads = $"sim_{campaign.Id}_ag1" }),
                        CreatedAt = now,
                        UpdatedAt = now
                    };
                    db.AdSets.Add(simAdSet);
                    await db.SaveChangesAsync(ct);
                    adSets.Add(simAdSet);
                }
                ...
                {
                    // Ensure a simulated ad exists for this ad set
                    var ads = await db.Ads
                        .Where(a => a.AdSetId == adSet.Id && a.CompanyId == companyId && a.Status != "archived")
                        .ToListAsync(ct);
                    ...
                    if (ads.Count == 0)
                    {
                        var simAd = new Ad
                        {
                            ...
                            Status = "active",
                            GoogleAdId = $"sim_{adSet.Id}_ad1",
                            PlatformAdIds = JsonSerializer.SerializeToElement(new { google_ads = $"sim_{adSet.Id}_ad1" }),
                            CreatedAt = now,
                            UpdatedAt = now
                        };
                        db.Ads.Add(simAd);
                    ...
                    {
                        // Build the set of hours already covered at ad level to avoid duplicates
                        var windowStart = now.AddHours(-48);
                        var existingHours = await db.AdMetrics
                            .Where(m => m.CampaignId == campaign.Id && m.AdSetId == adSet.Id && m.AdId == ad.Id
                                     && m.Platform == "google_ads" && m.Date >= windowStart)
                            .Select(m => m.Date)
                            .ToHashSetAsync(ct);

                        // Backfill the past 48 hours so the dashboard shows data immediately
                        for (int h = 47; h >= 0; h--)
                        {
                            var hourBucket = DateTime.SpecifyKind(now.Date.AddHours(now.Hour - h), DateTimeKind.Utc);
                            if (existingHours.Contains(hourBucket)) continue;
                            ...
                            var metric = new AdMetric
                            {
                                ...
                                Spend = Math.Round((decimal)(Random.Shared.NextDouble() * (double)(campaign.DailyBudget ?? 100) / 24), 2),
                                Conversions = Random.Shared.Next(0, 10),
                                FetchedAt = now
                            };
        ...
        private async Task<string?> GetAccessTokenAsync(CompanyAdAccount account, CancellationToken ct)
        {
            ...
            try
            {
                ...
                var token = json.TryGetProperty("access_token", out var tok) ? tok.GetString() : null;
        ...
        // loginCustomerId: the manager account making the call on behalf of customerId (standard MCC pattern).
        private async Task<List<JsonElement>> ExecuteGaqlAsync(string customerId, string developerToken,
            string accessToken, string query, CancellationToken ct, string? loginCustomerId = null)
        {
            ...
            _logger.LogDebug("[GOOGLE SYNC] GAQL response ({Bytes} bytes): {Preview}",
                responseBody.Length, responseBody.Length > 300 ? responseBody[..300] + "…" : responseBody);

            // searchStream returns a JSON array of SearchGoogleAdsStreamResponse objects.
            // Each object has a "results" array. Parse the whole body at once.
            try
            {
            ...
            {
                // Fall back to line-by-line for non-standard NDJSON responses
            ...
            _logger.LogInformation("[GOOGLE SYNC] GAQL parsed {Count} rows for customer {CustomerId}", results.Count, customerId);
            return results;
        }
