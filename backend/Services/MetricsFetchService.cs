...
namespace Backend.Services
{
    ...
    public class MetricsFetchService : BackgroundService
    {
        ...
        private async Task FetchPlatformMetrics(AppDbContext db, Campaign campaign, CompanyAdAccount account, CancellationToken ct)
        {
            ...
            // Check per-hour so each new hour produces a fresh simulated row.
            var currentHour = DateTime.SpecifyKind(now.Date.AddHours(now.Hour), DateTimeKind.Utc);
            var nextHour = currentHour.AddHours(1);
            var existing = await db.AdMetrics.AnyAsync(
                m => m.CampaignId == campaign.Id && m.Platform == account.Platform && m.Date >= currentHour && m.Date < nextHour,
                ct);

            if (existing)
            {
                _logger.LogDebug("Metrics already fetched this hour for campaign {Id} on {Platform}", campaign.Id, account.Platform);
                return;
            }

            GenerateSimulatedMetrics(db, campaign, account, currentHour);
        }
        ...
        {
            // Facebook does not return reach at hourly granularity (it is a unique-user count
            // that cannot be split by hour). Fetch daily reach separately so we can distribute
            // it proportionally across the hourly rows for each (ad, day) pair.
            var dailyReachFields = $"ad_id,reach,date_start";
            var dailyReachLast7dUrl = $"https://graph.facebook.com/v19.0/{adAccountId}/insights" +
                $"?level={level}&fields={dailyReachFields}" +
                $"&date_preset=last_28d&time_increment=1&limit=1000&access_token={accessToken}";
            var dailyReachTodayUrl = $"https://graph.facebook.com/v19.0/{adAccountId}/insights" +
                $"?level={level}&fields={dailyReachFields}" +
                $"&date_preset=today&time_increment=1&limit=500&access_token={accessToken}";

            var dailyReachRows = new List<JsonElement>();
            try
            {
                var r1 = await FetchGraphCollectionAsync(dailyReachLast7dUrl, ct);
                var r2 = await FetchGraphCollectionAsync(dailyReachTodayUrl, ct);
                dailyReachRows.AddRange(r1);
                dailyReachRows.AddRange(r2);
            }
            catch (Exception ex)
            {
                _logger.LogWarning(ex, "[FB SYNC] Could not fetch daily reach for {AdAccountId}; reach will be 0", adAccountId);
            }

            // Key: (adId, dateStart) → daily reach value
            var dailyReachLookup = new Dictionary<(string AdId, string DateStart), long>();
            foreach (var row in dailyReachRows)
            {
                var adId = GetString(row, "ad_id") ?? "";
                var dateStart = GetString(row, "date_start") ?? "";
                var reach = ParseLong(row, "reach");
                if (!string.IsNullOrEmpty(adId) && !string.IsNullOrEmpty(dateStart) && reach > 0)
                    dailyReachLookup[(adId, dateStart)] = reach;
            }

            // Primary: hourly breakdown for last 28 days — real per-hour buckets only.
            var hourlyUrl = $"https://graph.facebook.com/v19.0/{adAccountId}/insights" +
                $"?level={level}&fields={fields}" +
                $"&breakdowns=hourly_stats_aggregated_by_advertiser_time_zone" +
                $"&date_preset=last_28d&limit=1000&access_token={accessToken}";

            var hourlyRows = await FetchGraphCollectionAsync(hourlyUrl, ct);

            // date_preset=last_28d excludes the current day — fetch today separately so
            ...
            // Count hourly rows per (adId, date) so we can distribute daily reach evenly.
            var hourlyCountPerDay = allHourlyRows
                .GroupBy(r => (AdId: GetString(r, "ad_id") ?? "", Date: GetString(r, "date_start") ?? ""))
                .ToDictionary(g => g.Key, g => g.Count());

            _logger.LogInformation("[FB SYNC] Fetched {Last7d} last_7d + {Today} today rows, {DailyReach} daily-reach rows for {AdAccountId}",
                hourlyRows.Count, todayRows.Count, dailyReachRows.Count, adAccountId);
            ...
            foreach (var row in allHourlyRows)
            {
                ...
                // reach is not available at hourly granularity — distribute the day's total evenly.
                var fbAdId = GetString(row, "ad_id") ?? "";
                var dateStart = GetString(row, "date_start") ?? "";
                var dailyReach = dailyReachLookup.TryGetValue((fbAdId, dateStart), out var dr) ? dr : 0L;
                var hourCount = hourlyCountPerDay.TryGetValue((fbAdId, dateStart), out var hc) && hc > 0 ? hc : 1;
                var reach = dailyReach / hourCount;
