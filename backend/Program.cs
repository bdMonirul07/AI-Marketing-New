...
app.MapPost("/api/campaigns/{id}/generate-assets", async (AppDbContext db, HttpContext ctx, int id, GenerateAssetsRequest? req, IConfiguration config) =>
{
    ...
    var imagenClient = new HttpClient { Timeout = TimeSpan.FromSeconds(120) };

    // Maps aspect ratio to Pollinations width/height
    static (int w, int h) RatioToDimensions(string ratio) => ratio switch
    {
        "9:16"  => (768, 1360),
        "16:9"  => (1360, 768),
        "3:4"   => (768, 1024),
    ...
    async Task<string> GenerateImageAsync(string platform, string brief, string style, string aspectRatio)
    {
        try
        {
            var platformContext = platform switch
            {
                ...
                "youtube"  => "YouTube banner ad",
                "google"   => "Google Display ad",
                _          => "social media ad"
            };

            // Step 1: Use Gemini to craft a rich image prompt (optional — falls back gracefully)
            string imagePrompt = $"Professional {style} {platformContext}: {brief}, high quality, photorealistic, marketing photography";
                ...
                try
                {
                    ...
                    var promptBody = new
                    {
                        contents = new[] { new { parts = new[] { new { text =
                            $"Create a vivid image generation prompt for a {platformContext}. " +
                            $"Campaign brief: \"{brief}\". Visual style: {style}. " +
                            $"Photorealistic, professional, visually striking. " +
                            $"Output only the image prompt, max 100 words." } } } }
                    };
            ...
            // Step 2: Generate image via Pollinations.ai (free, no API key required)
            var (width, height) = RatioToDimensions(aspectRatio);
            var seed = Random.Shared.Next(1, 99999);
            var encodedPrompt = Uri.EscapeDataString(imagePrompt);
            var pollinationsUrl = $"https://image.pollinations.ai/prompt/{encodedPrompt}?width={width}&height={height}&seed={seed}&nologo=true&enhance=true";

            var imageBytes = await imagenClient.GetByteArrayAsync(pollinationsUrl);
            if (imageBytes.Length == 0) return "";

            var filename = $"gen_{platform}_{Guid.NewGuid():N}.jpg";
        ...
        {
            Console.WriteLine($"[IMAGE GEN] Failed for {platform}: {ex.Message}");
            return "";
        }
    }
...
// Mark ppp_queue items as dispatched (removes them from Platform Selection view)
app.MapPatch("/api/ppp/queue/dispatched", async (AppDbContext db, HttpContext ctx, List<int> numericIds) =>
{
    var companyId = ctx.GetRequiredCompanyId();
    var items = await db.PppQueue
        .Where(q => q.CompanyId == companyId && numericIds.Contains(q.Id))
        .ToListAsync();
...
app.MapGet("/api/analytics/kpis", async (AppDbContext db, HttpContext ctx, string? platform, int? hours) =>
{
    ...
    // Anchor to the latest row with real activity so zero-delivery sync rows don't push
    // the window past all actual data (Facebook inserts 0-impression rows for the current hour).
    var baseQ = db.AdMetrics.Where(m => m.CompanyId == companyId);
    if (!string.IsNullOrEmpty(platform) && platform != "all")
        baseQ = baseQ.Where(m => m.Platform == platform);
    var maxDate = await baseQ
        .Where(m => m.Impressions > 0 || m.Clicks > 0 || m.Spend > 0)
        .MaxAsync(m => (DateTime?)m.Date) ?? now;
...
app.MapGet("/api/analytics/trends", async (AppDbContext db, HttpContext ctx, string? platform, int? hours, string? metric, string? split) =>
{
    ...
    decimal ValueFor(long impressions, long reach, long videoViews, long likes, long comments, long shares, long saves, double watchSec, int followers) =>
        metricKey switch
        {
            "reach"      => reach,
    ...
    // Build a complete hourly timeline so gaps render as 0 instead of blank canvas.
    var nowBst = HourKey(DateTime.UtcNow);
    var allLabels = Enumerable.Range(0, windowHours)
        .Select(i => nowBst.AddHours(i - windowHours + 1))
        .ToList();
    ...
    if (splitBy == "split")
    {
        var byPlatformHour = raw
            ...
            .ToDictionary(
                g => g.Key,
                g => g.Aggregate(0m, (acc, r) => acc + ValueFor(r.Impressions, r.Reach, r.VideoViews, r.Likes, r.Comments, r.Shares, r.Saves, r.WatchSec, r.Followers)));

        var platforms = byPlatformHour.Keys.Select(k => k.Platform).Distinct().ToList();
        var series = platforms.Select(p => new
        {
            platform = p,
            data = allLabels.Select(l => byPlatformHour.TryGetValue(new { Platform = p, Hour = l }, out var v) ? v : 0m).ToList()
        });
        return Results.Ok(new { labels = allLabels, series });
    }
    else
    {
        var byHourDict = raw
        ...
        {
            labels = allLabels,
            series = new[] { new { platform = platform ?? "all", data = allLabels.Select(l => byHourDict.TryGetValue(l, out var v) ? v : 0m).ToList() } }
        });
    }
}).RequireAuthorization();
...
app.MapGet("/api/analytics/posts", async (AppDbContext db, HttpContext ctx, string? platform, int? hours, string? sort) =>
{
    ...
    // Anchor "last hour" to the most recent data with real delivery, not to UtcNow.
    // Facebook inserts zero-impression rows for the current in-progress hour, which would
    // otherwise push the window past all actual data.
    var anchor = await metricsQ
        .Where(m => m.Impressions > 0 || m.Clicks > 0)
        .MaxAsync(m => (DateTime?)m.Date) ?? now;
    var lastHourStart = anchor.AddHours(-1);
    var prevHourStart = anchor.AddHours(-2);
...
app.MapGet("/api/analytics/virality", async (AppDbContext db, HttpContext ctx, string? platform) =>
{
    ...
    var baseQ = db.AdMetrics.Where(m => m.CompanyId == companyId && m.AdId != null);
    if (!string.IsNullOrEmpty(platform) && platform != "all")
        baseQ = baseQ.Where(m => m.Platform == platform);

    // Anchor windows to the latest real-delivery row, not to UtcNow.
    var anchor = await baseQ
        .Where(m => m.Impressions > 0 || m.Clicks > 0)
        .MaxAsync(m => (DateTime?)m.Date) ?? now;
    var recentStart = anchor.AddHours(-2);
    var baselineStart = anchor.AddHours(-8);

    var q = baseQ.Where(m => m.Date >= baselineStart);
