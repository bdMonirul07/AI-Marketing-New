...
const themeButtons = {
  ...
  glass: document.getElementById('theme-glass'),
...
function setTheme(theme) {
  document.body.classList.remove('theme-light', 'theme-blue', 'theme-glass', 'theme-windo')
  if (theme !== 'dark') {
...
${l2.length ? '\n[PSYCHOLOGICAL PILLARS]\n' + l2.map((q, i) => `Q: ${q}\nA: ${state.marketingData.level2Answers[i] || 'N/A'}`).join('\n') : ''}`
  ...
  document.getElementById('proceed-to-studio').onclick = async () => {
    ...
    try {
      ...
      // 3. Navigate to Studio immediately — don't block on image generation
      state.pendingImageGeneration = enabledPlatforms.length > 0 ? campaignId : null
      switchScreen('Studio')

      // 4. Fire-and-forget image generation in background
      if (enabledPlatforms.length > 0) {
        apiFetch(`/campaigns/${campaignId}/generate-assets`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ brief })
        }).then(() => {
          showNotification('✅ Images ready! Click "Refresh Assets" in Studio to see them.', 'success')
        }).catch(e => {
          console.warn('[generate-assets background]', e)
        })
      }
    } catch (error) {
      console.error('Error saving config:', error)
      showNotification('Failed to save platform config. Check console for details.', 'error')
...
async function renderStudioScreen() {
  ...
  try {
      ...
      <div class="space-y-6">
          <div class="flex justify-between items-end">
              ...
              <div class="flex gap-3">
                  <button id="refresh-studio-btn" class="px-4 py-2.5 bg-emerald-600/20 text-emerald-400 border border-emerald-500/30 font-bold rounded-lg transition-all uppercase tracking-widest text-xs hover:bg-emerald-500 hover:text-white">
                      ↻ Refresh Assets
                  </button>
          ...
          ${state.pendingImageGeneration ? `
          <div class="bg-emerald-900/20 border border-emerald-500/30 rounded-xl p-4 flex items-center gap-3">
              <div class="animate-spin rounded-full h-5 w-5 border-t-2 border-b-2 border-emerald-400 flex-shrink-0"></div>
              <p class="text-xs font-bold text-emerald-400">AI is generating images in the background... Click <strong>↻ Refresh Assets</strong> after 1–2 minutes to see them.</p>
          </div>` : ''}
    ...
    // Refresh Studio assets
    document.getElementById('refresh-studio-btn').onclick = () => renderStudioScreen()
...
async function renderApprovedAssetsScreen() {
  ...
  // Separate active (not yet deployed/rejected) from completed
  const activeQueue = queue.filter(i => i.status !== 'deployed' && i.status !== 'rejected')
  const completedQueue = queue.filter(i => i.status === 'deployed' || i.status === 'rejected')

  // Group active items by platform for display
  const byPlatform = {}
  ...
  const budgetSet        = activeQueue.filter(i => ['budget_configured', 'ready_for_approval'].includes(i.status)).length
  const readyForApproval = activeQueue.filter(i => i.status === 'budget_configured').length
  ...
  contentContainer.innerHTML = `
    <div class="space-y-6 pb-32">
      ...
      ${activeQueue.length === 0 ? `
        <div class="bg-[var(--card-bg)] border border-dashed border-[var(--border-color)] p-20 rounded-3xl flex flex-col items-center justify-center text-center space-y-4">
          <span class="text-5xl opacity-20">📋</span>
          <p class="text-gray-500 font-bold">No active assets in the PPP queue.<br>
      ...
      <!-- Completed / Deployed items (collapsed) -->
      ${completedQueue.length > 0 ? `
        <details class="bg-[var(--card-bg)] border border-[var(--border-color)] rounded-2xl overflow-hidden">
          <summary class="p-4 cursor-pointer flex items-center justify-between select-none hover:bg-white/5 transition-all">
            <span class="text-xs font-black uppercase tracking-widest text-gray-500">✓ Completed / Deployed (${completedQueue.length})</span>
            <span class="text-[10px] text-gray-600 uppercase">Click to expand</span>
          </summary>
          <div class="divide-y divide-[var(--border-color)]">
            ${completedQueue.map(item => {
              const sc = statusConfig[item.status] || statusConfig.deployed
              return `
                <div class="p-4 flex items-center gap-4 opacity-50">
                  <div class="w-12 h-12 bg-black rounded-lg overflow-hidden flex-shrink-0">
                    ${item.type === 'video'
                      ? `<video src="${item.url || ''}" class="w-full h-full object-cover"></video>`
                      : `<img src="${item.url || ''}" class="w-full h-full object-cover" onerror="this.parentElement.innerHTML='<div class=\\'w-full h-full flex items-center justify-center text-lg\\'>🖼</div>'">`}
                  </div>
                  <div class="flex-1 min-w-0">
                    <p class="text-xs font-bold uppercase truncate">${item.title || item.id}</p>
                    <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[9px] font-black uppercase ${sc.bg} ${sc.color} mt-1">${sc.icon} ${sc.label}</span>
                  </div>
                </div>
              `
            }).join('')}
      ...
      ${activeQueue.some(i => i.status === 'budget_configured' || i.status === 'ready_for_approval') ? `
  ...
  try {
    ...
    if (Array.isArray(queue)) {
      assets = queue
        ...
        .map(i => ({ id: i.id, numericId: i.numericId, url: i.url, type: i.type, title: i.title, platform: i.platform }))
    }
  } catch (e) {
...
${JSON.stringify(combinedPreviewPayload, null, 2)}
    ...
    async function executeActualDispatch() {
      ...
      try {
        ...
        // Mark dispatched items in DB so they disappear from Platform Selection
        const dispatchedIds = assets.map(a => a.numericId).filter(Boolean)
        if (dispatchedIds.length > 0) {
          try {
            await apiFetch('/ppp/queue/dispatched', {
              method: 'PATCH',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify(dispatchedIds)
            })
          } catch (e) {
            console.warn('Could not mark items as dispatched:', e.message)
          }
        }
...
let _platformServicePollTimer = null
function _stopPlatformServicePoll() { if (_platformServicePollTimer) { clearInterval(_platformServicePollTimer); _platformServicePollTimer = null } }
...
async function renderPlatformServiceScreen() {
  ...
  try {
        ...
        <div class="bg-[var(--card-bg)] border border-[var(--border-color)] rounded-2xl p-6 space-y-5">
          ...
          <div class="flex items-start justify-between gap-3">
            ...
            <div class="flex flex-wrap gap-1.5 justify-end">
              ...
              <span id="${prefix}-running-badge" class="px-2.5 py-1 rounded-full text-[9px] font-bold uppercase tracking-widest ${isRunning ? 'bg-cyan-500/10 text-cyan-400 border border-cyan-500/20' : 'bg-gray-500/10 text-gray-400 border border-gray-500/20'}">${isRunning ? 'Fetching' : 'Idle'}</span>
          ...
          <div class="grid grid-cols-2 gap-2">
            ${[
              ...
              { label: 'Last Run', value: `<span id="${prefix}-lastrun">${formatPlatformServiceDate(service.lastRunCompletedAt)}</span>` },
              { label: 'Outcome', value: `<span id="${prefix}-outcome-badge" class="inline-flex px-2 py-0.5 rounded-full text-[9px] font-bold uppercase ${getPlatformServiceOutcomePillClass(lastOutcome)}">${lastOutcome}</span>` }
            ].map(c => `
    ...
    // Auto-poll every 4 s while either service is actively fetching, stop when both are idle.
    const fbRunning = !!(fbData.status?.isRunning)
    const googleRunning = !!(googleData.status?.isRunning)
    if (fbRunning || googleRunning) {
      _platformServicePollTimer = setInterval(async () => {
        try {
          ...
          const stillRunning = !!(d1.status?.isRunning) || !!(d2.status?.isRunning)
          if (!stillRunning) _stopPlatformServicePoll()
          // Silently patch status cards without full re-render to avoid wiping event listeners
          const patch = (prefix, data) => {
            const s = data.status || {}
            const isRunning = !!s.isRunning
            const isEnabled = !!s.isEnabled
            const hasPending = !!s.hasPendingManualRun
            const outcome = s.lastRunOutcome || 'idle'
            const el = (id) => document.getElementById(`${prefix}-${id}`)
            if (el('running-badge')) el('running-badge').textContent = isRunning ? 'Fetching' : 'Idle'
            if (el('outcome-badge')) el('outcome-badge').textContent = outcome
            if (el('lastrun')) el('lastrun').textContent = s.lastRunCompletedAt ? new Date(s.lastRunCompletedAt).toLocaleString() : '—'
          }
          patch('fb-svc', d1)
          patch('google-svc', d2)
          if (!stillRunning) renderPlatformServiceScreen()
        } catch { _stopPlatformServicePoll() }
      }, 4000)
    }
      ...
      <section class="bg-[var(--card-bg)] border border-[var(--border-color)] p-5 rounded-2xl">
        <div class="flex items-center justify-between flex-wrap gap-3 mb-4">
          ...
          <div id="ua-metricTabs" class="flex items-center gap-1 bg-[var(--bg-color)] border border-[var(--border-color)] rounded-lg p-1">
            ${['views','engagement','reach','watch','followers'].map((m,i) =>
              `<button data-metric="${m}" class="ua-tab text-xs px-3 py-1 rounded ${i===0?'bg-cyan-600 text-white':''}">${{views:'Views',engagement:'Engagement',reach:'Reach',watch:'Watch time',followers:'Followers'}[m]}</button>`
            ).join('')}
          </div>
  ...
  try {
    ...
    contentContainer.innerHTML = `
      <div class="space-y-8">
        ...
        <div class="space-y-3">
          <h3 class="text-[10px] font-black text-emerald-500 uppercase tracking-widest border-b border-emerald-500/20 pb-2">💰 PPP Budget Submissions — Ready for Deployment</h3>
          ...
          ${pppSubmissions.length === 0 ? `
            <div class="bg-[var(--card-bg)] border border-dashed border-[var(--border-color)] p-10 rounded-2xl flex flex-col items-center justify-center text-center space-y-2">
              ...
              <p class="text-gray-500 text-sm font-bold">No PPP submissions awaiting approval.<br><span class="text-xs font-medium opacity-50 uppercase tracking-tighter">PPP will submit budget-configured ads here for deployment.</span></p>
            </div>
